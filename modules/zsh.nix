...
{
  # Ensure user-local binaries are on PATH. This is where the native Claude Code
  # installer puts its self-updating `claude` shim, and it takes precedence over
  # ~/.nix-profile/bin by design — see modules/packages/agentic.nix.
  ...
  programs.zsh = {
    ...
    shellAliases = {
      ...
      lt = "eza --tree";
      cc = "noglob _cc";
      # nixpkgs ships the Zed CLI as `zeditor`; `zed` upstream is a Rust build tool name clash
      zed = "zeditor";
